...
export function ModalPanel({ title, triggerLabel, children, variant = "primary" }: { title: string; triggerLabel: string; children: React.ReactNode; variant?: "primary" | "ghost" }) {
  ...
  return <>
    <button type="button" onClick={() => dialogRef.current?.showModal()} className={clsx("rounded-xl px-4 py-2 text-sm font-semibold transition", variant === "primary" ? "bg-gradient-to-r from-rapid-blue to-rapid-cyan text-white shadow-lg shadow-blue-950/40" : "border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10")}>{triggerLabel}</button>
    <dialog ref={dialogRef} className="w-[min(96vw,1100px)] rounded-3xl border border-white/10 bg-slate-950 p-0 text-slate-100 shadow-2xl shadow-black/70 backdrop:bg-slate-950/75">
