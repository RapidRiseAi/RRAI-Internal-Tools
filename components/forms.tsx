...
export function QuoteForm({ clients, leads, services }: { clients: ClientOption[]; leads: LeadOption[]; services: ServiceOption[] }) {
  return <form action={upsertQuote} className="grid gap-4 md:grid-cols-3">
    ...
    <div className="flex items-end"><Button>Create quote</Button></div>
  </form>;
}

export function AcceptQuoteButton({ quote }: { quote: Quote }) {
  if (quote.status === "ACCEPTED") return null;
  return <form action={acceptQuote}><input type="hidden" name="id" value={quote.id} /><Button>Accept & create project</Button></form>;
}

export function ProjectForm({ clients, quotes, project }: { clients: ClientOption[]; quotes: QuoteOption[]; project?: Project }) {
  return <form action={upsertProject} className="grid gap-4 md:grid-cols-3">
...
export function SupportTicketForm({ clients, projects, users, ticket }: { clients: ClientOption[]; projects: ProjectOption[]; users: UserOption[]; ticket?: SupportTicket }) {
  return <form action={upsertSupportTicket} className="grid gap-4 md:grid-cols-3">{ticket?.id ? <input type="hidden" name="id" value={ticket.id} /> : null}<Field label="Title"><input className={inputClass} name="title" defaultValue={ticket?.title ?? ""} required /></Field><Field label="Category"><select className={inputClass} name="category" defaultValue={ticket?.category ?? "OTHER"}><Options values={ticketCategories} /></select></Field><Field label="Priority"><select className={inputClass} name="priority" defaultValue={ticket?.priority ?? "MEDIUM"}><Options values={priorities} /></select></Field><Field label="Status"><select className={inputClass} name="status" defaultValue={ticket?.status ?? "NEW"}><Options values={ticketStatuses} /></select></Field><Field label="Client"><select className={inputClass} name="clientId" defaultValue={ticket?.client_id ?? ""}><option value="">No client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.company_name}</option>)}</select></Field><Field label="Project"><select className={inputClass} name="projectId" defaultValue={ticket?.project_id ?? ""}><option value="">No project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></Field><Field label="Assigned to"><select className={inputClass} name="assignedToId" defaultValue={ticket?.assigned_to ?? ""}><option value="">Unassigned</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></Field><Field label="Internal notes"><textarea className={inputClass} name="internalNotes" defaultValue={ticket?.internal_notes ?? ""} /></Field><Field label="Resolution notes"><textarea className={inputClass} name="resolutionNotes" defaultValue={ticket?.resolution_notes ?? ""} /></Field><div className="md:col-span-3"><Button>{ticket?.id ? "Update ticket" : "Create ticket"}</Button></div></form>;
}

export function AffiliateForm() { return <form action={upsertAffiliate} className="grid gap-4 md:grid-cols-5"><Field label="Name"><input className={inputClass} name="name" required /></Field><Field label="Email"><input className={inputClass} name="email" type="email" required /></Field><Field label="Tracking code"><input className={inputClass} name="trackingCode" placeholder="Auto if empty" /></Field><Field label="Status"><select className={inputClass} name="status"><Options values={affiliateStatuses} /></select></Field><Field label="Commission %"><input className={inputClass} name="defaultCommissionRate" type="number" min="0" max="100" defaultValue="10" /></Field><input type="hidden" name="defaultCommissionType" value="ONCE_OFF" /><div className="md:col-span-5"><Button>Create affiliate</Button></div></form>; }

export function ReferralCommissionForms({ affiliates, leads, clients, quotes, projects, payments }: { affiliates: Affiliate[]; leads: LeadOption[]; clients: ClientOption[]; quotes: QuoteOption[]; projects: ProjectOption[]; payments: Payment[] }) { return <div className="grid gap-4 xl:grid-cols-2"><Card><h3 className="mb-4 font-semibold text-white">Link referral</h3><form action={createReferral} className="grid gap-3"><Field label="Affiliate"><select className={inputClass} name="affiliateId">{affiliates.map((affiliate) => <option key={affiliate.id} value={affiliate.id}>{affiliate.name}</option>)}</select></Field><Field label="Lead"><select className={inputClass} name="leadId"><option value="">No lead</option>{leads.map((lead) => <option key={lead.id} value={lead.id}>{lead.company_name}</option>)}</select></Field><Field label="Client"><select className={inputClass} name="clientId"><option value="">No client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.company_name}</option>)}</select></Field><input type="hidden" name="status" value="PENDING" /><Button>Create referral</Button></form></Card><Card><h3 className="mb-4 font-semibold text-white">Record commission</h3><form action={createCommission} className="grid gap-3"><Field label="Affiliate"><select className={inputClass} name="affiliateId">{affiliates.map((affiliate) => <option key={affiliate.id} value={affiliate.id}>{affiliate.name}</option>)}</select></Field><Field label="Quote"><select className={inputClass} name="quoteId"><option value="">No quote</option>{quotes.map((quote) => <option key={quote.id} value={quote.id}>{quote.title}</option>)}</select></Field><Field label="Project"><select className={inputClass} name="projectId"><option value="">No project</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></Field><Field label="Payment"><select className={inputClass} name="paymentId"><option value="">No payment</option>{payments.map((payment) => <option key={payment.id} value={payment.id}>{payment.reference ?? payment.id} ({payment.status})</option>)}</select></Field><Field label="Status"><select className={inputClass} name="status"><Options values={commissionStatuses} /></select></Field><Field label="Amount cents"><input className={inputClass} name="amountCents" type="number" min="1" required /></Field><input type="hidden" name="commissionType" value="ONCE_OFF" /><Button>Record commission</Button></form></Card></div>; }

export function CampaignForm() { return <form action={upsertCampaign} className="grid gap-4 md:grid-cols-4"><Field label="Name"><input className={inputClass} name="name" required /></Field><Field label="Platform"><input className={inputClass} name="platform" defaultValue="LinkedIn" /></Field><Field label="Target industry"><input className={inputClass} name="targetIndustry" /></Field><Field label="Offer"><input className={inputClass} name="offerMessage" /></Field>{[["numberContacted", "Contacted"], ["replies", "Replies"], ["callsBooked", "Calls"], ["quotesSent", "Quotes"], ["dealsClosed", "Deals"]].map(([name, label]) => <Field key={name} label={label}><input className={inputClass} name={name} type="number" min="0" defaultValue="0" /></Field>)}<div className="md:col-span-4"><Button>Create campaign</Button></div></form>; }
export function ContentItemForm() { return <form action={upsertContentItem} className="grid gap-4 md:grid-cols-4"><Field label="Title"><input className={inputClass} name="title" required /></Field><Field label="Status"><select className={inputClass} name="status"><Options values={contentStatuses} /></select></Field><Field label="Platform"><input className={inputClass} name="platform" /></Field><Field label="Post URL"><input className={inputClass} name="postUrl" /></Field><Field label="Leads"><input className={inputClass} name="leadsGenerated" type="number" min="0" defaultValue="0" /></Field><Field label="Performance notes"><textarea className={inputClass} name="performanceNotes" /></Field><div className="md:col-span-4"><Button>Create content item</Button></div></form>; }

export function KnowledgeBaseForm() { return <form action={upsertKnowledgeBaseItem} className="grid gap-4"><div className="grid gap-4 md:grid-cols-3"><Field label="Title"><input className={inputClass} name="title" required /></Field><Field label="Category"><select className={inputClass} name="category"><Options values={knowledgeCategories} /></select></Field><Field label="Visibility"><select className={inputClass} name="visibility"><option value="INTERNAL">Internal</option><option value="TEAM">Team</option><option value="PRIVATE">Private</option></select></Field></div><Field label="Article / SOP body"><textarea className={`${inputClass} min-h-40`} name="body" required /></Field><Button>Add article</Button></form>; }

export function NoteForm({ entityType, entityId, clientId, leadId, projectId, redirectTo }: { entityType: string; entityId: string; clientId?: string; leadId?: string; projectId?: string; redirectTo: string }) { return <form action={addNote} className="grid gap-3"><input type="hidden" name="entityType" value={entityType} /><input type="hidden" name="entityId" value={entityId} /><input type="hidden" name="clientId" value={clientId ?? ""} /><input type="hidden" name="leadId" value={leadId ?? ""} /><input type="hidden" name="projectId" value={projectId ?? ""} /><input type="hidden" name="redirectTo" value={redirectTo} /><Field label="Add note"><textarea className={inputClass} name="body" required /></Field><Button>Add note</Button></form>; }
export function FileRecordForm({ entityType, entityId, clientId, projectId, redirectTo }: { entityType: string; entityId: string; clientId?: string; projectId?: string; redirectTo: string }) { return <form action={addFileRecord} className="grid gap-3"><input type="hidden" name="entityType" value={entityType} /><input type="hidden" name="entityId" value={entityId} /><input type="hidden" name="clientId" value={clientId ?? ""} /><input type="hidden" name="projectId" value={projectId ?? ""} /><input type="hidden" name="redirectTo" value={redirectTo} /><Field label="File name"><input className={inputClass} name="filename" required /></Field><Field label="Private storage URL / document link"><input className={inputClass} name="url" type="url" required /></Field><Field label="MIME type"><input className={inputClass} name="mimeType" placeholder="application/pdf" /></Field><Button>Add file link</Button></form>; }
export function ChecklistTemplateForm({ services, templates }: { services: ServiceOption[]; templates: { id: string; name: string }[] }) { return <div className="grid gap-4 xl:grid-cols-2"><form action={createChecklistTemplate} className="grid gap-3"><h3 className="font-semibold text-white">Create checklist template</h3><Field label="Name"><input className={inputClass} name="name" required /></Field><Field label="Service"><select className={inputClass} name="serviceId"><option value="">General</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></Field><Field label="Description"><textarea className={inputClass} name="description" /></Field><Field label="First item"><input className={inputClass} name="firstItemTitle" /></Field><Button>Create template</Button></form><form action={createChecklistItem} className="grid gap-3"><h3 className="font-semibold text-white">Add checklist item</h3><Field label="Template"><select className={inputClass} name="templateId">{templates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field><Field label="Item title"><input className={inputClass} name="title" required /></Field><Field label="Description"><textarea className={inputClass} name="description" /></Field><Field label="Sort order"><input className={inputClass} name="sortOrder" type="number" min="0" defaultValue="0" /></Field><Button>Add item</Button></form></div>; }
