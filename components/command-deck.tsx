  ...
  return (
    <DeckCard padding={dense ? "p-3" : "p-4"}>
      <div className="flex items-start justify-between gap-2">
        <p className={clsx("font-display font-semibold uppercase tracking-[0.12em] text-deck-muted", dense ? "text-[0.6rem]" : "text-[0.64rem]")}>{label}</p>
        {href ? <OverflowLink href={href} label={`Open ${label}`} /> : null}
      </div>
      <div className={clsx("flex items-center", dense ? "mt-2 gap-2.5" : "mt-3 gap-4")}>
        <RadialRing value={ringValue} size={dense ? 44 : 64} stroke={dense ? 4 : 6}>
          <span className={clsx("font-mono font-semibold text-deck-text", dense ? "text-[0.6rem]" : "text-[0.8rem]")}>{Math.round(ringValue)}%</span>
        </RadialRing>
        <div className="min-w-0">
          <div className="flex items-baseline gap-1">
            <span className={clsx("font-mono font-bold leading-none text-deck-text", dense ? "text-xl" : "text-3xl")}>{value}</span>
...
export function ListPanel({ title, right, viewAllHref, viewAllLabel = "View all", fill = false, children }: { title: string; right?: ReactNode; viewAllHref?: string; viewAllLabel?: string; fill?: boolean; children: ReactNode }) {
  return (
    <DeckCard padding="p-0" className={clsx("flex flex-col overflow-hidden", fill && "h-full")}>
      ...
      <div className={clsx("flex-1 divide-y divide-hairline", fill && "min-h-0 overflow-y-auto")}>{children}</div>
      {viewAllHref ? (
