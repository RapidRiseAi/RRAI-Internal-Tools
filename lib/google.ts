...
export async function upsertGoogleConnection(input: { userId: string; googleUser: GoogleUser; tokens: TokenResponse }) {
  ...
  const payload = {
    ...
    scopes: input.tokens.scope ? input.tokens.scope.split(" ") : scopes,
...
export async function syncTaskToGoogleCalendar(taskId: string) {
  ...
  const { data: task } = await getSupabaseAdmin().from("tasks").select("id,title,description,due_date,assigned_to,google_calendar_event_id").eq("id", taskId).maybeSingle();
  if (!task?.assigned_to || !task.due_date) return;
  const accessToken = await accessTokenForUser(task.assigned_to as string);
  ...
  const eventId = task.google_calendar_event_id as string | null;
  const response = await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events${eventId ? `/${eventId}` : ""}`, {
  ...
  if (event.id) await getSupabaseAdmin().from("tasks").update({ google_calendar_event_id: event.id, google_calendar_event_url: event.htmlLink ?? null, google_calendar_synced_at: new Date().toISOString() }).eq("id", taskId);
}
