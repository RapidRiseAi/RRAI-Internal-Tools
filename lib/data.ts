...
export async function listLeads() { if (!hasSupabaseConfig()) return [] as Lead[]; const { data } = await getSupabaseAdmin().from("leads").select("*, assignee:users!leads_assigned_to_fkey(id,name)").is("archived_at", null).is("converted_client_id", null).order("updated_at", { ascending: false }); return (data ?? []) as Lead[]; }
export async function listAllLeadsForReporting() { if (!hasSupabaseConfig()) return [] as Lead[]; const { data } = await getSupabaseAdmin().from("leads").select("*, assignee:users!leads_assigned_to_fkey(id,name)").order("updated_at", { ascending: false }); return (data ?? []) as Lead[]; }
...
export async function getTask(id: string) { if (!hasSupabaseConfig()) return null; const { data } = await getSupabaseAdmin().from("tasks").select("*, assignee:users!tasks_assigned_to_fkey(id,name)").eq("id", id).maybeSingle(); return data as Task | null; }
...
export async function listFiles(filters: { leadId?: string; clientId?: string; projectId?: string; knowledgeBaseItemId?: string; knowledgeBaseItemIds?: string[]; entityId?: string }) { if (!hasSupabaseConfig()) return [] as FileRecord[]; if (filters.knowledgeBaseItemIds && !filters.knowledgeBaseItemIds.length) return [] as FileRecord[]; let query = getSupabaseAdmin().from("files").select("*").order("created_at", { ascending: false }); if (filters.leadId) query = query.eq("lead_id", filters.leadId); if (filters.clientId) query = query.eq("client_id", filters.clientId); if (filters.projectId) query = query.eq("project_id", filters.projectId); if (filters.knowledgeBaseItemId) query = query.eq("knowledge_base_item_id", filters.knowledgeBaseItemId); if (filters.knowledgeBaseItemIds?.length) query = query.in("knowledge_base_item_id", filters.knowledgeBaseItemIds); if (filters.entityId) query = query.eq("entity_id", filters.entityId); const { data } = await query; return (data ?? []) as FileRecord[]; }
...
export async function listInvoiceItems(invoiceId: string) { if (!hasSupabaseConfig()) return [] as import("./types").InvoiceItem[]; const { data } = await getSupabaseAdmin().from("invoice_items").select("*").eq("invoice_id", invoiceId).order("sort_order"); return (data ?? []) as import("./types").InvoiceItem[]; }
