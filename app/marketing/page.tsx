...
export default async function Marketing() {
  ...
  const [campaigns, content] = await Promise.all([
    genericList<Campaign>("campaigns"),
    genericList<ContentItem>("content_items"),
  ]);
  return (
    <AppShell>
      <PageHeader
        ...
        description="Track results on the main page and keep campaign/content creation in pop-ups."
        actions={
          <>
            <ModalPanel
        ...
        <div className="grid gap-4 md:grid-cols-5">
          {[
            ...
            [
              "Contacted",
              campaigns.reduce(
                (sum, campaign) => sum + campaign.number_contacted,
                0,
              ),
            ],
            [
              "Replies",
              campaigns.reduce((sum, campaign) => sum + campaign.replies, 0),
            ],
            [
              "Calls booked",
              campaigns.reduce(
                (sum, campaign) => sum + campaign.calls_booked,
                0,
              ),
            ],
        ...
        <Card>
          ...
          {campaigns.length ? (
              ...
              {campaigns.map((campaign) => (
                ...
                >
                  ...
                  <p className="text-sm text-slate-400">
                    {campaign.platform} • {campaign.replies} replies •{" "}
                    {campaign.deals_closed} deals
                  </p>
                </div>
              ))}
            </div>
            ...
            <EmptyState
              ...
              body="Use Add campaign to start tracking outreach."
            />
          )}
        </Card>
        <Card>
          ...
          {content.length ? (
              ...
              {content.map((item) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-slate-400">
                      {item.platform ?? "Content"} • {item.leads_generated}{" "}
                      leads
                    </p>
                  </div>
            ...
            <EmptyState
              ...
              body="Use Add content to record posts, pages or assets."
            />
          )}
        </Card>
      </div>
