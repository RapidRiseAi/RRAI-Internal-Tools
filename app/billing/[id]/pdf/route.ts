...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: invoiceItems } = await supabase.from("invoice_items").select("*").eq("invoice_id", id).order("sort_order");
  ...
  const pdf = brandedPdf({
    ...
    items: (invoiceItems?.length ? invoiceItems.map((item) => ({ description: item.description, quantity: item.quantity, amount: money(item.unit_amount_cents), status: money(item.quantity * item.unit_amount_cents) })) : [{ description: invoice.quote_id ? "Accepted quote invoice" : "Professional services", amount: money(invoice.amount_cents), status: invoice.status }]),
