...
export async function GET(
  ...
  const pdf = brandedPdf({
    ...
    meta: [
      ...
      `Valid until: ${dateShort(quote.valid_until)}`,
      `Quote ID: ${quote.quote_number}`,
    ],
    items: (items ?? []).map((item) => ({
      description: item.description,
      quantity: item.quantity,
      onceOff: money(item.once_off_cents),
      monthly: money(item.monthly_cents),
    })),
    totals: [
      `Once-off: ${money(quote.once_off_total_cents)}`,
      `Monthly: ${money(quote.monthly_total_cents)}`,
      "Excludes custom changes not listed above",
    ],
